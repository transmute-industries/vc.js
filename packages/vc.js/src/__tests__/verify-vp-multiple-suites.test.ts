    ...
    const verification = await vcjs.ld.verify({
        ...
        documentLoader: async (iri:string)=>{
            ...
            if (iri.startsWith('did:key:z6M')){
                const { didDocument } = await ed25519.driver.resolve(iri, {accept: 'application/did+ld+json'})
                return {
                    documentUrl: iri,
            ...
            if (iri.startsWith('did:key:z5T')){
                const { didDocument } = await bls12381.driver.resolve(iri, {accept: 'application/did+ld+json'})
                return {
                    documentUrl: iri,
                    document: didDocument
                } 
